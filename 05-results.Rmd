...
Then we try to figure out how the rest continuous variables, such as number of units, area, and history of a property, impact the sales price, so we use a scatter plot matrix colored by borough to find the correlations. The result shows that both the number of units and history of a property have no correlation with the sales price, so we omit them from figure 7. 

On the other hand, the sales price shows correlation with the square feet, but only in several boroughs. In Queens and Staten, both the gross square feet and the land square feet are positively correlated to the sales price. In Brooklyn, only the land square feet is positively correlated to the sales price.
